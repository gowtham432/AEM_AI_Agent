...
def generate_sling_model_with_rag(fields, user_context):
    ...
    if user_context:
        ...
        if has_tab_mention:
            # Parse which fields should go in which tab
            multifield_in_separate = any(phrase in context_lower for phrase in [
                'multifield in separate tab',
                'multifield in a separate tab', 
                'multifield in different tab',
                'separate tab for multifield'
            ])
...
{'MULTIFIELD IN SEPARATE TAB DETECTED: Put all non-multifield component fields in "Properties" tab, and multifield in a separate "Items" tab.' if multifield_in_separate else 'Analyze the context to determine tab organization.'}

Instructions:
1. How many tabs are needed (typically 2 if multifield in separate tab)
2. What each tab should be named:
   - Default first tab: "Properties" (for regular component fields)
   - Default multifield tab: "Items" or based on multifield label
   - Or use context clues for custom names
3. Which fields belong in which tab:
   - If "multifield in separate tab": All component fields EXCEPT multifield go in first tab, multifield goes in second tab
   - If specific tab names mentioned: distribute fields as specified in context
4. CRITICAL: Use the exact tab structure from the Dialog Template:
   - tabs → items → <tabNode jcr:title="Display Name"> → items → columns → items → column → items → [fields here]
5. Tab node names: lowercase/camelCase (e.g., properties, itemsTab, configuration)
6. Tab titles (jcr:title): Display-friendly (e.g., "Properties", "Items", "Configuration")
7. EACH tab must have the full structure: items → columns → items → column → items, then put fields inside that innermost items node
8. DO NOT make the multifield itself a tab - wrap it inside a proper tab structure

Example CORRECT structure for "multifield in separate tab":
<tabs>
    <items>
            ...
            <items>
                <columns>
                    <items>
                        <column>
                            <items>
                                <!-- ALL non-multifield component fields here -->
                                <text.../> 
                                <dropdown.../>
                                <checkbox.../>
                            </items>
...
<tabs>
    <items>
        ...
        <items (multifield directly as tab)>  <!-- WRONG! -->
    </items>
</tabs>
"""
